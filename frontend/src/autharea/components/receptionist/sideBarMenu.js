export const receptionistItems = [
  {
    title: "New Reservation",
    icon: "bi-person-add",
    path: "/receptionist/reservation/new",
  },
  {
    title: "All Reservation",
    icon: "bi-cassette-fill",
    path: "/receptionist/reservations",
  },
  {
    title: "Reservation with Old Guest",
    icon: "bi-database",
    path: "/receptionist/reservation-with-old-guest",
  },
  {
    title: "Free Rooms",
    icon: "bi-kanban-fill",
    path: "/receptionist/free-rooms",
  },
  {
    title: "Create Rooms",
    icon: "bi-kanban-fill",
    path: "/receptionist/create-rooms",
  },
  {
    title: "Change Password",
    icon: "bi-lock",
    path: "/receptionist/change-password",
  },
  {
    title: "Logout",
    icon: "bi-power",
    path: "/logout",
  },
];
export const restaurantItems = [
  {
    title: "New Order",
    icon: "bi-person-add",
    path: "/restaurant/order/new",
  },
  {
    title: "All Orders",
    icon: "bi-cassette-fill",
    path: "/restaurant/orders",
  },
  {
    title: "New Menu",
    icon: "bi-database",
    path: "/restaurant/new-menu",
  },
  {
    title: "Menu List",
    icon: "bi-database",
    path: "/restaurant/menu",
  },
  {
    title: "Change Password",
    icon: "bi-lock",
    path: "/restaurant/change-password",
  },
  {
    title: "Logout",
    icon: "bi-power",
    path: "/logout",
  },
];

export const kitchenItems = [
  {
    title: "All Orders",
    icon: "bi-arrows-fullscreen",
    path: "/kitchen/orders?backUrl=today",
  },
  {
    title: "Request For Stock",
    icon: "bi-app-indicator",
    path: "/kitchen/request-for-stock",
  },
  {
    title: "All Stock Requests",
    icon: "bi-database",
    path: "/kitchen/all-stocks",
  },
  {
    title: "Stock Dispatches",
    icon: "bi-cassette-fill",
    path: "/kitchen/stock-dispatches",
  },
  {
    title: "Change Password",
    icon: "bi-lock",
    path: "/kitchen/change-password",
  },
  {
    title: "Logout",
    icon: "bi-power",
    path: "/logout",
  },
];

export const storeKeeperItems = [
  {
    title: "New Stock",
    icon: "bi-arrows-fullscreen",
    path: "/store-keeper/stock/new",
  },
  {
    title: "All Stocks",
    icon: "bi-database",
    path: "/store-keeper/stock?backUrl=today",
  },
  {
    title: "New Dispatch",
    icon: "bi-arrows-fullscreen",
    path: "/store-keeper/dispatch/new",
  },
  {
    title: "All Stock Dispatched",
    icon: "bi-app-indicator",
    path: "/store-keeper/stock-dispatches?backUrl=today",
  },
  {
    title: "All Stock Requests",
    icon: "bi-database",
    path: "/store-keeper/all-stock",
  },
  {
    title: "Change Password",
    icon: "bi-lock",
    path: "/store-keeper/change-password",
  },
  {
    title: "Logout",
    icon: "bi-power",
    path: "/logout",
  },
];
export const barItems = [
  {
    title: "New Order",
    icon: "bi-arrows-fullscreen",
    path: "/bar/new-order",
  },
  {
    title: "All Orders",
    icon: "bi-database",
    path: "/bar/orders?backUrl=today",
  },
  {
    title: "New Stock Request",
    icon: "bi-arrows-fullscreen",
    path: "/bar/request-for-stock",
  },
  {
    title: "All Stock Request",
    icon: "bi-app-indicator",
    path: "/bar/all-stock-request",
  },
  {
    title: "All Dispatched Stocks",
    icon: "bi-app-indicator",
    path: "/bar/stock-dispatches",
  },
  {
    title: "Change Password",
    icon: "bi-lock",
    path: "/bar/change-password",
  },
  {
    title: "Logout",
    icon: "bi-power",
    path: "/logout",
  },
];
export const maintenanceItems = [
  {
    title: "Upload Used Stocks",
    icon: "bi-arrows-fullscreen",
    path: "/maintenance/create-used-stock",
  },
  {
    title: "All Used Stocks",
    icon: "bi-database",
    path: "/maintenance/all-uploaded-stocks",
  },
  {
    title: "New Stock Request",
    icon: "bi-cassette-fill",
    path: "/maintenance/request-for-stock",
  },
  {
    title: "All Stock Request",
    icon: "bi-app-indicator",
    path: "/maintenance/all-stock-request",
  },
  {
    title: "Dispatched Stocks",
    icon: "bi-kanban-fill",
    path: "/maintenance/dispatched-stocks",
  },
  {
    title: "Change Password",
    icon: "bi-lock",
    path: "/maintenance/change-password",
  },
  {
    title: "Logout",
    icon: "bi-power",
    path: "/logout",
  },
];
export const laundryItems = [
  {
    title: "Upload Used Stocks",
    icon: "bi-arrows-fullscreen",
    path: "/laundry/create-used-stock",
  },
  {
    title: "All Used Stocks",
    icon: "bi-database",
    path: "/laundry/all-uploaded-stocks",
  },
  {
    title: "New Stock Request",
    icon: "bi-cassette-fill",
    path: "/laundry/request-for-stock",
  },
  {
    title: "All Stock Request",
    icon: "bi-app-indicator",
    path: "/laundry/all-stock-request",
  },
  {
    title: "Dispatched Stocks",
    icon: "bi-kanban-fill",
    path: "/laundry/dispatched-stocks",
  },
  {
    title: "Change Password",
    icon: "bi-lock",
    path: "/laundry/change-password",
  },
  {
    title: "Logout",
    icon: "bi-power",
    path: "/logout",
  },
];

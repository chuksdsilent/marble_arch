                                    ...
                                    {fields.map(({ key, name, ...restField }) => (
                                        <Space key={key} style={{ display: 'flex', marginBottom: 8 }} align="baseline">
                                            ...
                                            <Form.Item
                                                style={{ width: "20rem" }}
                                                {...restField}
                                                name={[name, (currentUser.data.role == "restaurant" ? "menuId" : "dispatchId")]}
                                                rules={[{ required: true, message: "Menu Item is required" }]}>
                                                <Select placeholder="Select Menu">
                                                    {dataSource && dataSource.map(data => (
                                                        <Select.Option key={data._id} value={data._id}>{currentUser.data.role == "restaurant" ? data.name : data.stockId.name}</Select.Option>
                                                    ))}
                                                </Select>
                                            </Form.Item>
                                            <Form.Item

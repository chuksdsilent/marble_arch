...
const UpdateStock = () => {
    ...
    useEffect(async () => {
        try {
            const response = await StockServices.getStock(id);
    ...
    const onFinish = async (values) => {
        ...
        setSubmitting(true)
        try {
            const response = await StockServices.updateStock(id, values);
            form.resetFields();
            setReservation(response.data)
            success("Stock Successfully.", "top-right", 5000);
            setSubmitting(false)
            navigate("/manager/stock-inventory")

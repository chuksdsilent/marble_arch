    ...
    useEffect(async () => {
        try {
            if (currentUser.data.role === "receptionist") {
                const responseData = await BookingServices.todayBooking();
                setDataSource(responseData.data)
            } else {
                const responseData = (params === "today") ? await BookingServices.todayBooking() : await BookingServices.allBooking()
                setDataSource(responseData.data)
